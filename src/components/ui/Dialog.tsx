...
export const Dialog = ({ open, onClose, size = "md", className, children }: DialogProps) => (
  <HeadlessDialog open={open} onClose={onClose} className="relative z-9999">
    ...
    <div className="fixed inset-0 flex items-center justify-center p-4">
      <DialogPanel
        transition
        className={cn(SIZE_CLASSES[size], "rounded-xl transition-all duration-300 ease-out data-closed:scale-90 data-closed:opacity-0", className)}
      >
        {children}
      </DialogPanel>
    </div>
...
export const DialogHeader = ({ className, ...rest }: DivProps) => (
  <div className={cn("flex shrink-0 items-center p-4 text-2xl font-semibold", className)} {...rest} />
);

export const DialogBody = ({ className, ...rest }: DivProps) => (
  <div className={cn("relative p-4 font-light leading-relaxed", className)} {...rest} />
);

export const DialogFooter = ({ className, ...rest }: DivProps) => (
  <div className={cn("flex shrink-0 flex-wrap items-center justify-end p-4", className)} {...rest} />
);
