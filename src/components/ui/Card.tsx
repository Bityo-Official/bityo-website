...
export const Card = ({ className = "", ...rest }: DivProps) => (
  <div className={`relative flex flex-col rounded-xl bg-white bg-clip-border text-gray-700 shadow-md ${className}`} {...rest} />
);
...
export const CardHeader = ({ floated = true, shadow = true, className = "", ...rest }: CardHeaderProps) => (
  <div
    className={`relative mx-4 mt-4 overflow-hidden rounded-xl bg-white bg-clip-border text-gray-700 ${floated ? "-mt-6" : ""} ${shadow ? "shadow-lg" : "shadow-none"} ${className}`}
    {...rest}
  />
);
